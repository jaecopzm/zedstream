...
func (h *Handler) CreatePlaylist(w http.ResponseWriter, r *http.Request) {
	...
	`, id.New(), userID, body.Title, desc, body.IsPublic,
...
// GetPlaylist returns a playlist and its tracks.
//
...
func (h *Handler) GetPlaylist(w http.ResponseWriter, r *http.Request) {
	...
	`, playlistID).Scan(&p.ID, &p.UserID, &p.Title, &p.Description, &p.CoverURL, &p.IsPublic, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
		response.NotFound(w, "playlist not found")
		return
	}
	...
	if !p.IsPublic && p.UserID != userID {
		response.Forbidden(w, "playlist is private")
		return
	}

	rows, err := h.db.Query(r.Context(), `
		SELECT t.id, t.artist_id, t.title, a.stage_name, t.cover_url, t.duration_sec,
		       t.play_count, t.album_id, al.title
...
// loadTrackCollaborators batch-loads featured artists for the given track IDs
// and returns a map of track id -> collaborator list ({artist_id, stage_name}).
func (h *Handler) loadTrackCollaborators(ctx context.Context, trackIDs []string) map[string][]map[string]string {
	...
	rows, err := h.db.Query(ctx, `
		SELECT tc.track_id, tc.artist_id, a.stage_name
	...
	defer rows.Close()
	for rows.Next() {
		var trackID, artistID, stageName string
		if err := rows.Scan(&trackID, &artistID, &stageName); err != nil {
			slog.Warn("scan collaborator", "error", err)
			continue
		}
		result[trackID] = append(result[trackID], map[string]string{
			"artist_id": artistID,
...
func (h *Handler) GetMyLikes(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.db.Query(r.Context(), `
		SELECT t.id, t.artist_id, t.title, a.stage_name, t.cover_url, t.duration_sec,
		       t.play_count, t.album_id, al.title, l.created_at
		FROM likes l
	...
	for rows.Next() {
		lt := LikedTrack{}
		if err := rows.Scan(&lt.ID, &lt.ArtistID, &lt.Title, &lt.ArtistName, &lt.CoverURL, &lt.DurationSec, &lt.PlayCount, &lt.AlbumID, &lt.AlbumName, &lt.LikedAt); err != nil {
	...
	for i, lt := range liked {
		trackIDs[i] = lt.ID
	}
	colabs := h.loadTrackCollaborators(r.Context(), trackIDs)
	for i := range liked {
		if c, ok := colabs[liked[i].ID]; ok {
			liked[i].Collaborators = c
		}
	}
...
func (h *Handler) AddComment(w http.ResponseWriter, r *http.Request) {
	...
	`, id.New(), trackID, userID, body.Body,
