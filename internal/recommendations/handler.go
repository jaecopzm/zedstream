...
func (h *Handler) GetRadio(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.db.Query(r.Context(), `
		WITH source AS (
		    SELECT genre_id, artist_id FROM tracks WHERE id = $1
		),
		recent_plays AS (
		...
		SELECT t.id, t.title, a.stage_name, t.cover_url, t.duration_sec
		FROM tracks t
		JOIN artists a ON a.id = t.artist_id
		CROSS JOIN source s
		WHERE t.status = 'published'
		  ...
		  AND (t.genre_id = s.genre_id OR t.artist_id = s.artist_id)
	...
	for rows.Next() {
		t := RadioTrack{}
		if err := rows.Scan(&t.ID, &t.Title, &t.ArtistName, &t.CoverURL, &t.DurationSec); err == nil {
